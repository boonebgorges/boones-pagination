...
class BBG_CPT_Pag {
	 ...
	 *   ?per_page=10&paged=1
	 *
	 *   - "per_page" - is the 'get_per_page_key' argument; if you're running multiple
	 *     custom paginated loops, you'll want to set this to a different value for each
	 *      paginated loop to prevent pagination collision
	 *   - "10"       - is the 'per_page' argument; the number of items to fetch
	 *   - "paged"    - is the 'get_paged_key' argument; the key to fetch the current page of
	 *     results
	 *
	 ...
	 * Sets up query vars.
	 *
	 * I recommend that you instantiate the class right away when you start rendering the page,
	 ...
	 * query, call the setup_query() method manually, in order to populate the class with
	 * query-specific data.
	 *
